...
bool Block::checkValid(uint32_t pageIndex, uint32_t idx) {
  if (ioUnitInPage == 1) {
...
bool Block::checkErased(uint32_t pageIndex, uint32_t idx) {
  if (ioUnitInPage == 1) {
...
bool Block::checkClean(uint32_t pageIndex, uint32_t idx) {
  return checkErased(pageIndex, idx) && !checkValid(pageIndex, idx);
}
